...
gini_index <- rep(NA, dim(df)[1])
for (i in 1: dim(df)[1])
{
  gini_index[i] <- ineq(df[i,1:10])
}
...
for (i in 1:194)
{
  gini[i] <- ineq(design[i,1:10], type = "Gini")
}
gini

design <- as.data.frame(cbind(design, gini))
names(design) <- c('b1','b2','b3','b4','b5','b6','b7','b8','b9','b10','evenness','celaantal','v1','v2','v3','v4','v5','v6','v7','v8','v9','v10','c1','c2','c3','c4','c5','c6','c7','c8','c9','c10','gini_index')
...
plot(design$gini_index, design$celaantal, xlab="evenness", ylab="cellcount")
grid(nx=3,ny=3)
